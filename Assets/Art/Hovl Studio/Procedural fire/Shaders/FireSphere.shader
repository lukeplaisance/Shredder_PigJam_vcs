Shader "EGA/Particles/FireSphere"
{
    Properties
    {
        ...
        _StartFrequency("Start Frequency", Range(1, 10)) = 4
        _Frequency("Frequency", Range(1, 20)) = 10
        _Amplitude("Amplitude", Range(0, 2)) = 1
        [ToggleUI] _Usedepth("Use depth?", Range(0, 1)) = 0
        _Depthpower("Depth power", Range(0.1, 10)) = 1
        [ToggleUI] _Useblack("Use black", Range(0, 1)) = 0
        ...
        SubShader
        {
            Tags { "RenderType" = "Transparent"   "RenderPipeline" = "UniversalPipeline" "Queue" = "Transparent" "RenderPriority" = "2000" }
            Cull Back
            CGPROGRAM
            ...
            {
                fixed4 temp_output_121_0 = (_Emission * _Color * i.vertexColor);
                fixed2 temp_output_8_0 = ((((fixed2(0.2, 0) * _Time.y) + (i.uv_tex3coord).xy + i.uv_tex3coord.z) * _StartFrequency));
                fixed2 break18 = floor(temp_output_8_0);
                float temp_output_21_0 = (break18.x + (break18.y * 57.0));
                fixed2 temp_output_10_0 = frac(temp_output_8_0);
                fixed2 temp_cast_1 = fixed2(3.0, 3.0);
                fixed2 break17 = (temp_output_10_0 * temp_output_10_0 * (temp_cast_1 - (temp_output_10_0 * 2.0)));
                fixed lerpResult39 = lerp(frac((473.5 * sin(temp_output_21_0))), frac((473.5 * sin((1.0 + temp_output_21_0)))), break17.x);
                fixed lerpResult38 = lerp(frac((473.5 * sin((57.0 + temp_output_21_0)))), frac((473.5 * sin((58.0 + temp_output_21_0)))), break17.x);
                fixed lerpResult40 = lerp(lerpResult39, lerpResult38, break17.y);
                fixed3 temp_output_51_0 = (((fixed3((fixed2(0.5, 0.5) * _Time.y), 0.0) + (i.uv_tex3coord * (lerpResult40 * _Amplitude))) + i.uv_tex3coord.z) * _Frequency);
                fixed3 break87 = floor(temp_output_51_0);
                float temp_output_90_0 = (break87.x + (break87.y * 57.0));
                fixed3 temp_output_52_0 = frac(temp_output_51_0);
                fixed3 temp_cast_3 = fixed3(3.0, 3.0, 3.0);
                fixed3 break110 = (temp_output_52_0 * temp_output_52_0 * (temp_cast_3 - (temp_output_52_0 * 2.0)));
                fixed lerpResult109 = lerp(frac((473.5 * sin(temp_output_90_0))), frac((473.5 * sin((1.0 + temp_output_90_0)))), break110.x);
                fixed lerpResult105 = lerp(frac((473.5 * sin((57.0 + temp_output_90_0)))), frac((473.5 * sin((58.0 + temp_output_90_0)))), break110.x);
                fixed lerpResult106 = lerp(lerpResult109, lerpResult105, break110.y);
                fixed Amp114 = _Amplitude;
                fixed4 tex2DNode117 = tex2D(_MainTex, (i.uv_tex3coord + (0.2 * (lerpResult106 * Amp114))).xy);
                o.Emission = lerp(temp_output_121_0, (temp_output_121_0 * tex2DNode117), _Useblack).rgb;
                fixed4 clampResult132 = clamp((i.vertexColor.a * tex2DNode117 * _Opacity), fixed4(0, 0, 0, 0), fixed4(1, 1, 1, 1));
                fixed4 ase_screenPos = fixed4(i.screenPos.xyz, i.screenPos.w + 0.00000000001);
                fixed4 ase_screenPosNorm = ase_screenPos / ase_screenPos.w;
                ase_screenPosNorm.z = ((UNITY_NEAR_CLIP_VALUE >= 0) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5);
                fixed screenDepth137 = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, ase_screenPosNorm.xy));
                float distanceDepth137 = abs((screenDepth137 - LinearEyeDepth(ase_screenPosNorm.z)) / (_Depthpower));
                fixed clampResult136 = clamp(distanceDepth137, 0.0, 1.0);
